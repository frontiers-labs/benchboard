...
const state = {
  ...
  suite: 'runtime' as Suite, metric: '', search: '', benchmark: '', compiler: '', references: true, threshold: 5,
...
const metricInfo: Record<string, { label: string; description: string }> = {
  Ir: { label: 'Instructions executed', description: 'Instructions executed under Cachegrind. This counts work, not elapsed time.' },
  Bc: { label: 'Conditional branches', description: 'Conditional branches executed.' },
  Bcm: { label: 'Branch mispredictions', description: "Conditional branches mispredicted by Cachegrind's simulated branch predictor." },
  Dr: { label: 'Data reads', description: 'Memory read operations.' },
  Dw: { label: 'Data writes', description: 'Memory write operations.' },
  I1mr: { label: 'L1 instruction cache misses', description: 'Instruction reads missed in the simulated first-level cache.' },
  ILmr: { label: 'Last-level instruction cache misses', description: 'Instruction reads missed in the simulated last-level cache.' },
  D1mr: { label: 'L1 data read misses', description: 'Data reads missed in the simulated first-level cache.' },
  D1mw: { label: 'L1 data write misses', description: 'Data writes missed in the simulated first-level cache.' },
  DLmr: { label: 'Last-level data read misses', description: 'Data reads missed in the simulated last-level cache.' },
  DLmw: { label: 'Last-level data write misses', description: 'Data writes missed in the simulated last-level cache.' },
  latency: { label: 'Elapsed time', description: 'Wall-clock time for the measured work.' },
  user_cpu_ns: { label: 'User CPU time', description: 'CPU time spent running user code.' },
  system_cpu_ns: { label: 'System CPU time', description: 'CPU time spent in the operating system.' },
  peak_process_rss_bytes: { label: 'Peak memory', description: 'Largest resident memory footprint of the measured process.' },
};
const metricLabel = (metric: string) => metricInfo[metric]?.label || metric;
...
const unitValue = (value: number | null, unit: string) => {
  ...
  if (unit === 'count') return nfmt(value, 0);
...
function reconcile() {
  ...
  if (!metrics.has(state.metric)) state.metric = metrics.has('latency') ? 'latency' : metrics.has('Ir') ? 'Ir' : [...metrics].sort()[0] || '';
  persist();
}
...
function option(value: string, current: string, label = value) { return `<option value="${esc(value)}" ${value === current ? 'selected' : ''}>${esc(label)}</option>`; }
function header() {
  const run = selectedRun();
  const metrics = [...new Set(run?.measurements.filter(row => row.suite === state.suite).map(row => row.metric) || [])].sort();
  const selectedUnit = run?.measurements.find(row => row.suite === state.suite && row.metric === state.metric)?.unit;
  ...
  <main><div class="suite-nav" role="group" aria-label="Suite">${([['compile', 'Compiling'], ['runtime', 'Program execution'], ['criterion', 'Microbenchmarks']] as [Suite, string][]).map(([suite, label]) => `<button data-suite="${suite}" class="${state.suite === suite ? 'active' : ''}" aria-pressed="${state.suite === suite}">${label}</button>`).join('')}</div>
  <div class="controls"><label>Metric<select id="metric">${metrics.map(metric => option(metric, state.metric, metricLabel(metric))).join('')}</select></label><label class="search">Benchmark<input id="search" type="search" placeholder="Search benchmark or compiler" value="${esc(state.search)}"></label><label>Baseline<select id="baseline"><option value="">None</option>${baselines.map(item => runOption(item, state.baseline)).join('')}</select></label><label>Current<select id="current">${state.runs.map(item => runOption(item, run?.id || '')).join('')}</select></label><label class="threshold">Threshold<input id="threshold" type="number" min="0" max="100" step="0.1" value="${state.threshold}" aria-label="Regression threshold percent"><span>%</span></label></div>
  <p class="metric-note">${esc(metricInfo[state.metric]?.description || 'Lower values are better for this metric.')} ${selectedUnit === 'count' ? 'These Cachegrind counts do not measure speed directly.' : 'Lower is better.'}${state.suite !== 'criterion' ? " A ratio above 1 means the row's compiler uses more of this metric than Clang or GCC; a dash means no matching reference measurement." : ''}</p>`;
}
function alerts() {
...
function changes() {
  ...
  const countText = state.comparing ? 'Comparing runs…' : !baselineRun() ? 'No compatible prior nightly yet. Compiler comparisons use the current run.' : comparison?.status === 'incomparable' ? esc(comparison.reason || 'Runs cannot be compared.') : comparison ? `${regressions} regressions${missing ? `, ${missing} missing` : ''} out of ${comparable.length} comparable across all suites · threshold ${nfmt(state.threshold, 1)}%${rows.length !== comparison.rows.length ? ` · ${shownRegressions} regressions shown` : ''}` : '';
